...
const modifiedIndex = index.map((e) => {
    return {
        id: e.id.substring(0, 3),
        value: e.value,
    }
})

const key_to_keep = ['id', 'value']

const redux = (arr) =>
    arr.map((e) =>
        key_to_keep.reduce((acc, cur) => {
            acc[cur] = e[cur].substring(0, 3)
            return acc
        }, {}),
    )

console.log('redux', redux(index))
